...
function spyropress_get_excerpt( $args = '' ) {
    ...
    if ( $link_to_post ) {
        ...
        $text .= '<div><a class="more" href="' . $permalink . '">' . $link_text . '</a></div>';
    }
...
function spyropress_wp_nav_menu_objects( $items, $args ) {
    ...
    $label = get_setting( 'translate' ) ? get_setting( 'translate-home', 'Home' ) : __( 'Home', 'spyropress' );
    
    $url = is_front_page() ? '#homepage' : home_url();
    $items = '<li><a href="' . $url . '">' . $label . '</a></li>' . $items;
